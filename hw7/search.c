...
struct node *move(struct node *cp,int a,int b,int x,int y,int dir) {
  ...
  for(i=0;i<N;i++){
    for(j=0;j<N;j++){
      if(i==x&&j==y)          //Move Zero to new position
        newp->board[i][j]==0;
      else if(i==a&&j==b)     //Move swap with the zero
        newp->board[i][j]=cp->board[x][y];
...
struct node *filter(struct node *succ,struct node *hp){ 
   ...
   struct node *sucitr=succ;
   printf("Filter\n");
   rsp=NULL;
   while(sucitr){
    int cont=0;
    //  printf("Thing");
     struct node *hpitr=hp;
     while(hpitr){
      // printf("Checking:\n");
      // print_a_node(sucitr);
      // print_a_node(hpitr);
      if(nodes_same(sucitr,hpitr)){
        
        printf("\nFiltering Out A Node\n");
        if(sucitr->next){
          lsp=sucitr->next;
          sucitr->next=sucitr->next->next;
          free(lsp);
          cont=1;
          break;
        }
